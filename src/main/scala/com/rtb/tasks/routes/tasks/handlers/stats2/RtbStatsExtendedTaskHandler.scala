package com.rtb.tasks.routes.tasks.handlers.stats2
 ...
 */
class RtbStatsExtendedTaskHandler(val config: Config) extends TaskHandler with ConfigSupport with LoggingSupport {
  ...
  private val listenerDruid   = CompletionListener(RtbStatsExtendedDruidRequestSuccessCount, RtbStatsExtendedDruidRequestFailureCount, countersHandler)

  override def handle(taskRequest: TaskRequest): Unit = {
    val query = getQuery(taskRequest)
    debug(s"running RtbStatsExtendedTaskHandler. query = $query")
    runQuery(query)
      .toFutureEither
      .foreach { res =>
  ...
  private def getQuery(taskRequest: TaskRequest): String = {
    ...
    s"""|
        |SELECT * from(select '$date' as "date", bid_date, ssp_id, dsp_endpoint_id, country, app_bundle, size, impression_types, endpoint_imp_type as dsp_endpoint_imp_type, dc_id, supply_site_id, supply_publisher_id, dsp_cid, dsp_crid, dsp_adomain, dsp_seat, dsp_buyer_id, rule_group_id, placement_id, bid_requests, bid_responses, dynamic_requests_sent, dynamic_bids, adm_requests, nurl_requests, burl_requests, beacon_views_count, ssp_imps_count, dynamic_imps_count, (cost * 1.0 / 1000) as cost, (ssp_cost * 1.0 / 1000) as ssp_cost, (dynamic_revenue * 1.0 / 1000) as dynamic_revenue, (dynamic_partner_revenue * 1.0 / 1000) as dynamic_partner_revenue, (revenue * 1.0 / 1000) as revenue from (
        |  select bid_date, ssp_id, dsp_endpoint_id, country, app_bundle, size, impression_types, endpoint_imp_type, dc_id, supply_site_id, supply_publisher_id, dsp_cid, dsp_crid, dsp_adomain, dsp_seat, dsp_buyer_id, rule_group_id, placement_id,
        ...
        |  and ssp_id < 50
        |  group by bid_date, ssp_id, dsp_endpoint_id, country, app_bundle, size, impression_types, endpoint_imp_type, dc_id, supply_site_id, supply_publisher_id, dsp_cid, dsp_crid, dsp_adomain, dsp_seat, dsp_buyer_id, rule_group_id, placement_id
        |))
