package com.rtb.tasks.routes.tasks.handlers.stats2.p3
...
import com.common.clients.kafka.KafkaProducerQueue.KafkaProducerConfig
...
/**
 * Created by Niv on 17/10/2024
 */
class RtbStatsExtendedP3TaskHandler(val config: Config) extends TaskHandler with ConfigSupport with LoggingSupport {
  ...
  private val listenerBq      = CompletionListener(RtbStatsExtendedP3BqRequestSuccessCount, RtbStatsExtendedP3BqRequestFailureCount, countersHandler)

  override def handle(taskRequest: TaskRequest): Unit = {
    val query = getQuery(taskRequest)
    debug(s"running RtbStatsExtendedP3TaskHandler. query = $query")
    runQuery(query)
      .toFutureEither
      .foreach { res =>
  ...
  private def getQuery(taskRequest: TaskRequest): String = {
    ...
    s"""|
        |SELECT * from(select '$date' as "date", bid_date, ssp_id, dsp_endpoint_id, country, app_bundle, size, impression_types, endpoint_imp_type as dsp_endpoint_imp_type, dc_id, supply_site_id, supply_publisher_id, dsp_response_imp_type, rule_group_id, placement_id, num_of_schain_nodes, ssp_response_imp_type, bid_requests, bid_responses, dynamic_requests_sent, dynamic_bids, adm_requests, nurl_requests, burl_requests, beacon_views_count, ssp_imps_count, dynamic_imps_count, (cost * 1.0 / 1000) as cost, (ssp_cost * 1.0 / 1000) as ssp_cost, (dynamic_revenue * 1.0 / 1000) as dynamic_revenue, (dynamic_partner_revenue * 1.0 / 1000) as dynamic_partner_revenue, (revenue * 1.0 / 1000) as revenue from (
        |  select bid_date, ssp_id, dsp_endpoint_id, country, app_bundle, size, impression_types, endpoint_imp_type, dc_id, supply_site_id, supply_publisher_id, dsp_response_imp_type, rule_group_id, placement_id, num_of_schain_nodes, ssp_response_imp_type,
        ...
        |  and ssp_id < 50
        |  group by bid_date, ssp_id, dsp_endpoint_id, country, app_bundle, size, impression_types, endpoint_imp_type, dc_id, supply_site_id, supply_publisher_id, dsp_response_imp_type, rule_group_id, placement_id, num_of_schain_nodes, ssp_response_imp_type
        |))
        |where ((cost > 0 or dynamic_requests_sent >= 1000 or dynamic_revenue > 0 or dynamic_imps_count > 0 or adm_requests > 0 or burl_requests > 0) or bid_requests >= 1000)
        |UNION ALL(
        |SELECT * from(select '$date' as "date", bid_date, ssp_id, dsp_endpoint_id, country, app_bundle, size, impression_types, endpoint_imp_type as dsp_endpoint_imp_type, dc_id, supply_site_id, supply_publisher_id, dsp_response_imp_type, rule_group_id, placement_id, num_of_schain_nodes, ssp_response_imp_type, bid_requests, bid_responses, dynamic_requests_sent, dynamic_bids, adm_requests, nurl_requests, burl_requests, beacon_views_count, ssp_imps_count, dynamic_imps_count, (cost * 1.0 / 1000) as cost, (ssp_cost * 1.0 / 1000) as ssp_cost, (dynamic_revenue * 1.0 / 1000) as dynamic_revenue, (dynamic_partner_revenue * 1.0 / 1000) as dynamic_partner_revenue, (revenue * 1.0 / 1000) as revenue from (
        |  select bid_date, ssp_id, dsp_endpoint_id, country, app_bundle, size, impression_types, endpoint_imp_type, dc_id, supply_site_id, supply_publisher_id, dsp_response_imp_type, rule_group_id, placement_id, num_of_schain_nodes, ssp_response_imp_type,
        |  sum(CASE WHEN beacon_type = '10' then "count" else 0 end) as bid_requests,
