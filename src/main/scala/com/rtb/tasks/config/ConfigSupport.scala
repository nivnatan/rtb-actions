...
import com.rtb.tasks.TasksServer
import com.rtb.tasks.utils.counters.Counters
...
case class Config(context: ActorContext)(implicit val system: ActorSystem, val ec: ExecutionContext, val classLoader: ClassLoader) {
  ...
  val countersHandler: CountersHandler          = CountersBuilder(TasksServer.name).withRtbStatsExtendedCounters().withServiceCounters(Counters.toSet).withListener(CountersListenerFluentd(fluentd, RtbConstants.RtbTasksMetricsLabel)).get
